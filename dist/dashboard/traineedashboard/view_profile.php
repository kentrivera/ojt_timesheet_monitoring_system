...
$image_path = '../admindashboard/' . $trainee['image'];

// Check if the image exists, otherwise use a default image
if (!file_exists($image_path) || empty($trainee['image'])) {
    $image_path = 'default.jpg'; // Ensure this default image exists in the same directory as this script
}
...
$total_logged_hours_query = "
    SELECT IFNULL(SUM(
        TIME_TO_SEC(TIMEDIFF(first_timeout, first_time_in)) + 
        IFNULL(TIME_TO_SEC(TIMEDIFF(second_timeout, second_time_in)), 0)
    ) / 3600, 0) AS total_logged_hours
    FROM studentloggeddata
    WHERE trainee_id = ?
";

$stmt = $conn->prepare($total_logged_hours_query);
$stmt->bind_param("i", $trainee_id);
$stmt->execute();
$total_hours_result = $stmt->get_result()->fetch_assoc();
$total_logged_hours = $total_hours_result['total_logged_hours'] ?? 0;
$remaining_hours = max(0, $trainee['required_hours'] - $total_logged_hours);
...
<body class="bg-gray-50">
    ...
    <div class="mb-10">
        <?php include '../traineedashboard/nav/top-nav.php'; ?>
    </div>
    ...
    <div class="flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8">
        <div class="w-full max-w-5xl p-0 bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="relative bg-gradient-to-r from-primary-dark to-primary h-24 sm:h-32 p-4 sm:p-6">
                <div class="flex justify-between items-center">
                    <a href="javascript:history.back()" class="inline-flex items-center gap-2 text-sm text-white bg-white/20 px-3 py-1 rounded-md hover:bg-white/30 transition">
                        <i class="fas fa-arrow-left"></i>
                    </a>
                    <h1 class="text-xl sm:text-3xl font-bold text-white text-center">Trainee Profile</h1>
                    <div class="w-10"></div> <!-- Spacer for flex justification -->
                </div>
            </div>

            <!-- Profile Card -->
            <div class="flex flex-col md:flex-row -mt-12 sm:-mt-16 px-4 sm:px-6 profile-section">
                <!-- Profile Image and Basic Info -->
                <div class="w-full md:w-1/3 flex flex-col items-center profile-info">
                    <div class="relative">
                        <img src="<?php echo $image_path; ?>" alt="Trainee Image"
                            class="w-24 h-24 sm:w-32 sm:h-32 border-4 border-white rounded-full shadow-md object-cover bg-gray-200">
                    </div>
                    <h2 class="mt-3 sm:mt-4 text-xl sm:text-2xl font-bold text-gray-800"><?php echo htmlspecialchars($trainee['name']); ?></h2>
                    ...
                    <div class="mt-4 sm:mt-6 w-full p-4 bg-gray-50 rounded-lg">
                ...
                <!-- Progress and Stats -->
                <div class="w-full md:w-2/3 pl-0 md:pl-6 mt-6 md:mt-0 stats-section">
                    <!-- Stats Cards -->
                    <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-100 mt-6 sm:mt-20">
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                            ...
                            <div class="bg-secondary-light/10 p-4 rounded-lg">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="text-lg font-medium <?php echo $this_week_hours > 0 ? 'text-secondary-dark' : 'text-gray-700'; ?>">
                            ...
                            <div class="bg-green-50 p-4 rounded-lg">
                                <p class="text-xs text-gray-600 mb-1">Total Logged</p>
                                <div class="flex items-center">
                                    <i class="fas fa-clock text-green-500 mr-2"></i>
                                    <span class="text-lg font-bold text-gray-800"><?php echo number_format($total_logged_hours, 1); ?></span>
                            ...
                            <div class="bg-blue-50 p-4 rounded-lg">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="text-lg font-bold text-gray-800"><?php echo number_format($remaining_hours, 1); ?></span>
                                    <span class="text-xs ml-1">hrs</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                        <!-- Weekly Report Card -->
                        <div class="bg-gradient-to-r from-secondary-light to-secondary p-4 sm:p-5 rounded-lg shadow-sm hover:shadow-md transition text-white">
                            <div class="flex items-start justify-between">
                                <div>
                                    <h3 class="font-bold text-base sm:text-lg mb-1">Weekly Report</h3>
                                    <p class="text-white/80 text-xs sm:text-sm mb-2 sm:mb-4">Week of <?php echo date('F j, Y', strtotime('monday this week')); ?></p>
                                </div>
                                <div class="text-white/90 bg-white/20 p-2 rounded-full">
                                    <i class="fas fa-file-alt"></i>
                                </div>
                            </div>
                            <a href="../traineedashboard/weekly_hours.php?id=<?php echo $trainee_id; ?>" class="mt-2 inline-block px-3 sm:px-4 py-1 sm:py-2 bg-white/20 hover:bg-white/30 rounded-lg text-xs sm:text-sm text-white transition">
                        ...
                        <div class="bg-gradient-to-r from-primary-light to-primary p-5 rounded-lg shadow-sm hover:shadow-md transition text-white">
                            ...
                            <a href="../traineedashboard/view_timesheet.php?id=<?php echo $trainee_id; ?>" class="mt-2 inline-block px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-sm text-white transition">
